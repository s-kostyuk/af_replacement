...
def eval_variant(
        net_type: str, af_name: str, start_ep: int, *, patched: bool = False
):
    batch_size = 64
    rand_seed = 42

    print(
        "Loading pre-trained {} KerasNet network with {}{} activation "
        "after {} epochs.".format(
            net_type, af_name, "" if net_type == "base" else "-like",
            start_ep
        )
    )

    path_net = get_file_name_net(
        net_type, af_name, start_ep, patched
    )

    dev = get_device()
    torch.manual_seed(rand_seed)

    _, test_set = get_cifar10_dataset(augment=True)
    ...
    net = create_net(net_type, af_name)
    net.to(device=dev)
